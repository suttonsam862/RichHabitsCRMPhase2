...
-- Corrected salesperson tables creation with proper constraint handling
-- Run this with: psql $DATABASE_URL -f fix_salesperson_tables_corrected.sql
...
-- Insert sample data with proper constraints
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'users') THEN
        -- Insert a sample salesperson user with required fields
        ...
        RAISE NOTICE 'Sample salesperson user and profile created';
    END IF;
END $$;
...
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'organizations') THEN
        -- Insert sample organization with required status field
        ...
        RAISE NOTICE 'Sample organization and assignment created';
    END IF;
END $$;
