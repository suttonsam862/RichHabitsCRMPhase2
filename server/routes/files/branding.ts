  ...
  asyncHandler(async (req: AuthedRequest, res) => {
    ...
    try {
      ...
      return HttpErrors.internalError(
        res,
        "Failed to generate signed upload URLs",
      );
    }
  }),
);
  ...
  asyncHandler(async (req: AuthedRequest, res) => {
    ...
    try {
      ...
      const keys = names.map(
        (name) => `org/${orgId}/branding/${safeName(name)}`,
      );
      ...
      logger.info(
        { fileCount: names.length, fileNames: names },
        "Deleted branding files",
      );
      sendNoContent(res);
    } catch (error) {
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      return HttpErrors.internalError(
        res,
        "Failed to update organization title card",
      );
    }
  }),
);
